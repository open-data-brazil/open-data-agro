...
// Client downloads datasets from the OECD SDMX REST API.
type Client struct {
	httpClient *http.Client
	maxRetries int
}

// NewClient creates an OECD HTTP client with retry and timeout defaults.
...
func (c *Client) download(ctx context.Context, sourceURL string) ([]byte, error) {
	...
	return nil, fmt.Errorf("oecd download failed after %d attempts: %w", c.maxRetries, lastErr)
}
