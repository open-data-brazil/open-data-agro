...
// DownloadResult holds a fetched INMET portal file and HTTP metadata.
...
// Client downloads files from the INMET portal.
type Client struct {
	httpClient *http.Client
	maxRetries int
}

// NewClient creates an INMET HTTP client with retry and timeout defaults.
...
// Download fetches bytes from a validated INMET portal URL.
...
func (c *Client) downloadOnce(ctx context.Context, sourceURL string) (*DownloadResult, error) {
	...
	req.Header.Set("Referer", "https://portal.inmet.gov.br/dadoshistoricos")
