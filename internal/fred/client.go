...
// Client downloads datasets from FRED public CSV endpoints.
type Client struct {
	httpClient *http.Client
	maxRetries int
}

// NewClient creates a FRED HTTP client.
...
func (c *Client) download(ctx context.Context, sourceURL string) ([]byte, error) {
	...
	return nil, fmt.Errorf("fred download failed after %d attempts: %w", c.maxRetries, lastErr)
}
