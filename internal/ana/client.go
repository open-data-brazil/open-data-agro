...
const defaultServiceBaseURL = "https://telemetriaws1.ana.gov.br/ServiceANA.asmx/HidroSerieHistorica"

// Client downloads hydrology series from the ANA HidroWeb SOAP service.
type Client struct {
	httpClient *http.Client
	maxRetries int
}

// NewClient creates an ANA HTTP client with retry and timeout defaults.
...
func (c *Client) download(ctx context.Context, sourceURL string) ([]byte, error) {
	...
	return nil, fmt.Errorf("ana download failed after %d attempts: %w", c.maxRetries, lastErr)
}
