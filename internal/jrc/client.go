...
// Client downloads datasets from JRC MARS / ASAP endpoints.
type Client struct {
	httpClient *http.Client
	maxRetries int
}

// NewClient creates a JRC HTTP client.
...
func (c *Client) download(ctx context.Context, sourceURL string) ([]byte, error) {
	...
	return nil, fmt.Errorf("jrc download failed after %d attempts: %w", c.maxRetries, lastErr)
}
