...
dbt-build-mercado-precos: dbt-deps
	cd dbt && LAKE_LOCAL_ROOT=$(LAKE_ABS) dbt build --profiles-dir . --select 'stg_conab__precos_semanal_uf+ stg_conab__precos_semanal_municipio+ stg_conab__precos_mensal_uf+'
...
conab-mercado-precos-mvp:
	...
	duckdb $(DUCKDB_PATH) -c "SELECT produto, uf, mes, valor_produto_kg FROM analytics.conab_precos_mensal_uf WHERE upper(trim(produto)) = 'SOJA' AND uf = 'MT' ORDER BY ano, mes LIMIT 5"
