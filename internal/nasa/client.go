...
// Client downloads datasets from NASA POWER API.
type Client struct {
	httpClient *http.Client
	maxRetries int
}

// NewClient creates a NASA POWER HTTP client.
...
func (c *Client) download(ctx context.Context, sourceURL string) ([]byte, error) {
	...
	return nil, fmt.Errorf("nasa download failed after %d attempts: %w", c.maxRetries, lastErr)
}
