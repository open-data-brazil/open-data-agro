...
// DownloadResult holds a fetched ANTAQ file and HTTP metadata.
...
// Client downloads files from ANTAQ official endpoints.
type Client struct {
	httpClient *http.Client
	maxRetries int
}

// NewClient creates an ANTAQ HTTP client with retry and timeout defaults.
...
// Download fetches bytes from a validated ANTAQ portal URL.
func (c *Client) Download(ctx context.Context, sourceURL string) (*DownloadResult, error) {
	...
	return nil, fmt.Errorf("antaq download failed after %d attempts: %w", c.maxRetries, lastErr)
}
