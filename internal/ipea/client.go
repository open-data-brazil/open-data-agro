...
const (
	...
	odata4Base  = "http://www.ipeadata.gov.br/api/odata4"
)

// DownloadResult holds a fetched IPEA OData response and HTTP metadata.
...
// Client downloads series from the IPEA OData4 API.
type Client struct {
...
// NewClient creates an IPEA HTTP client with retry and timeout defaults.
...
// Download fetches bytes from a validated IPEA OData URL.
...
func buildSeriesURL(seriesCode string) string {
	return fmt.Sprintf("%s/ValoresSerie(SERCODIGO='%s')", odata4Base, seriesCode)
}
