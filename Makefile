...
CI_BR_SOURCES_WAVE_3_LAKE ?= /tmp/br-sources-wave-3-ci-lake
...
br-sources-wave-3-mvp:
	...
	$(MAKE) analytics-init LAKE_LOCAL_ROOT=$(LAKE_ABS) DUCKDB_PATH=$(DUCKDB_PATH)
	$(DUCKDB_BIN) $(DUCKDB_PATH) -c "SELECT COUNT(*) FROM analytics.dnit_snv_rodovias_federais"
	$(DUCKDB_BIN) $(DUCKDB_PATH) -c "SELECT br, uf, codigo, jurisdicao FROM analytics.dnit_snv_rodovias_federais LIMIT 3"
	$(DUCKDB_BIN) $(DUCKDB_PATH) -c "SELECT COUNT(*) FROM analytics.ipea_series_macro_regionais"
...
re-enable-deferred-mvp:
	chmod +x scripts/ci/verify_deferred_reenable_probe.py
	python3 scripts/ci/verify_deferred_reenable_probe.py
	python3 scripts/ci/verify_wave5_discovery_probe.py
	go test ./internal/antaq/... ./internal/fao/... ./internal/usda/... ./internal/wto/... ./internal/mexico/... ./internal/noaa/... ./internal/ingest/ -run 'DeferredReenable|FlattenComercio|FlattenPSD|FlattenGATS|FlattenITSTrade|FlattenGPCC|SIAP'
	@echo "Phase 57: 0/9 deferred datasets re-enabled — blockers unchanged; see docs/DEFERRED-SOURCES.md"
...
analytics-crossing-mvp:
	...
	$(MAKE) dbt-build-analytics-crossing LAKE_LOCAL_ROOT=$(LAKE_LOCAL_ROOT)
	$(MAKE) analytics-init LAKE_LOCAL_ROOT=$(LAKE_ABS) DUCKDB_PATH=$(DUCKDB_PATH)
...
source-health-bot:
	go run ./cmd/source-health-bot

ci-source-health-bot:
	go test ./internal/sourceprobe/... -count=1
	OPEN_DATA_AGRO_ROOT=$(CURDIR) go run ./cmd/source-health-bot --report-dir /tmp/source-health-reports-ci

source-health-publish:
	bash scripts/source-health-publish.sh
