...
// Client downloads files from the U.S. EIA Open Data API.
type Client struct {
...
// DownloadResult holds a fetched EIA response and HTTP metadata.
...
// NewClient creates an EIA HTTP client with retry and timeout defaults.
...
// Download fetches bytes from a validated EIA URL.
...
func (c *Client) downloadOnce(ctx context.Context, sourceURL string) (*DownloadResult, error) {
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		...
		return nil, fmt.Errorf("unexpected status %d for %s: %s", resp.StatusCode, sourceURL, strings.TrimSpace(string(body)))
	}
